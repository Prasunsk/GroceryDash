...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.errors?.[0]?.msg || 'Unable to register');
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-md mx-auto mt-20 p-8 bg-white bg-opacity-90 rounded-xl shadow-2xl border border-yellow-600">
      <h2 className="text-2xl font-bold mb-6 text-center text-yellow-800">Create Account</h2>
      {error && <p className="text-red-600 mb-4 text-center">{error}</p>}
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block mb-2 text-yellow-700 font-semibold">Name</label>
          <input
            type="text"
            name="name"
            value={formData.name}
